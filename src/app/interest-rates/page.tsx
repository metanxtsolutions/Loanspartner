...
export default function RatesPage() {
  return (
    <>
      ...
      <PageHero crumbs={[{ name: "Interest rates", path: "/interest-rates" }]} eyebrow={`Indicative rates as of ${readableDate(asOf)}`} title={`Loan interest rates in India, ${policyRates.asOfLabel}`} lede="Ranges across public sector banks, private banks, housing finance companies and NBFCs on our panel. Where your rate lands inside a range depends on your profile; we tell you before you apply." />
      <Section tone="cream">
        <div className="overflow-x-auto rounded-card border border-line bg-white shadow-soft" data-reveal>
          <table className="w-full min-w-[860px] text-sm">
            <caption className="sr-only">Indicative interest rate, amount, tenure and processing fee for each loan product</caption>
            <thead className="bg-cream text-left text-xs uppercase tracking-wide text-mute">
              <tr><th scope="col" className="px-5 py-3 font-bold">Product</th><th scope="col" className="px-5 py-3 font-bold">Interest rate (p.a.)</th><th scope="col" className="px-5 py-3 font-bold">Loan amount</th><th scope="col" className="px-5 py-3 font-bold">Tenure</th><th scope="col" className="px-5 py-3 font-bold">Processing fee</th></tr>
            </thead>
            <tbody className="tnum">
              {products.map((p) => (
                <tr key={p.slug} className="border-t border-line align-top">
                  <th scope="row" className="px-5 py-4 text-left"><Link href={`/loans/${p.slug}`} className="font-bold text-ink-900 hover:text-verdant-700">{p.name}</Link><span className="mt-1 block max-w-xs text-xs font-normal text-mute">{p.rate.note}</span></th>
      ...
      <Section tone="paper">
        <div className="grid gap-8 lg:grid-cols-3">
          <Card className="p-6"><p className="eyebrow text-verdant-600">Benchmark</p><p className="mt-2 font-display text-3xl">Repo rate {repoRateLabel}</p><p className="mt-2 text-sm text-mute">The benchmark for repo-linked floating-rate loans as of {policyRates.asOfLabel}. Such home loans price at the repo rate plus a spread of roughly 2.1% to 2.75%, fixed at sanction. Confirm the current rate on the RBI website.</p></Card>

...
export default async function LenderPage({ params }: PageProps<"/lenders/[slug]">) {
  ...
  return (
    <>
      ...
      <PageHero
        crumbs={[
          { name: "Lending partners", path: "/lenders" },
          { name: l.name, path },
        ]}
        eyebrow={l.type}
        title={l.name}
        lede={
          <>
            <p>{l.summary}</p>
            <p className="mt-3">
              <span className="text-ink-900 font-bold">Best for:</span> {l.bestFor}
            </p>
          </>
        }
        aside={
          <div className="rounded-panel border-line shadow-lift border bg-white p-6">
            ...
            <p className="text-mute mt-1 text-sm">
              We check your profile against {l.shortName}'s policy and others on the panel before any application.
            </p>
            <div className="mt-5">
              <HeroForm products={productOptions} defaultProduct={l.products[0]} source={`lender:${l.slug}`} />
            </div>
          </div>
        }
      >
        <a
          ...
          className="text-brass-600 hover:text-brass-600 mt-6 inline-flex items-center gap-1.5 text-sm font-bold"
        >
        ...
        <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr]">
          <Card className="p-6">
            ...
            <p className="text-mute mt-6 text-xs leading-relaxed">
              {siteConfig.compliance.rateDisclaimer} Loan approval and terms are decided solely by {l.name} under its
              credit policy.
            </p>
          </Card>
          <div>
      ...
      {similar.length > 0 && (
        <Section tone="paper">
          <SectionHeader
            eyebrow="Compare"
            title={`Other ${l.type.toLowerCase()}s on the panel`}
            lede="The best lender for you depends on your profile. We compare before you apply."
          />
          <div className="mt-8">
            <LenderCards lenders={similar} />
          </div>
          <Link href="/lenders" className="text-brass-600 mt-6 inline-flex items-center gap-1 text-sm font-bold">
            All lending partners <ArrowRight className="size-3.5" />
          </Link>
        </Section>
      )}
      <CtaBand
        title={`Is ${l.shortName} the right lender for you?`}
        lede="Share your requirement and we will compare it against the whole panel, at no cost and with no bureau enquiry."
      />
    </>
  );
}

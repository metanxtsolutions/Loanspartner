...
export async function generateMetadata({ params }: PageProps<"/partner/[slug]">): Promise<Metadata> {
  ...
  return pageMetadata({
    ...
    path: `/partner/${slug}`,
    keywords: [
      `${p.name.toLowerCase()} DSA`,
      `${p.name.toLowerCase()} DSA registration`,
      `${p.name.toLowerCase()} DSA commission`,
      `${p.name.toLowerCase()} agent`,
      "loan DSA",
      "DSA partner program",
    ],
  });
...
export default async function ProductDsaPage({ params }: PageProps<"/partner/[slug]">) {
  ...
  const faqs = [
    {
      question: `How much does a ${p.name.toLowerCase()} DSA earn per file?`,
      answer: `Indicatively ${p.dsa.payoutFrom}% to ${p.dsa.payoutTo}% of the disbursed amount. On a typical ticket of ${p.dsa.ticketSize}, that is a meaningful payout per file. Your slab is stated in your agreement and reviewed quarterly.`,
    },
  ...
  return (
    <>
      <JsonLd
        data={[webPageSchema({ name: title, description, path, dateModified: p.updatedAt }), faqPageSchema(faqs)]}
      />
      <PageHero
        tone="ink"
        crumbs={[
          { name: "Partner programme", path: "/partner" },
          { name: `${p.name} DSA`, path },
        ]}
        eyebrow="Product programme"
        title={<>{p.name} DSA</>}
        lede={
          <>
            <p className="font-display text-2xl text-white/90 italic">{p.dsa.pitch}</p>
            <p className="mt-4">
              {description} We handle lender selection, documentation and follow-up; you bring the relationship.
            </p>
          </>
        }
        aside={
      ...
      >
        <div className="mt-8 flex flex-wrap gap-3">
          ...
          <ButtonLink href={`/loans/${p.slug}`} variant="outline-light" size="lg">
            Product details for borrowers
          </ButtonLink>
        </div>
      </PageHero>
      ...
      <Section tone="cream">
        <div className="grid gap-10 lg:grid-cols-2">
          <div>
            ...
            <ul className="mt-6 space-y-3">
              {p.benefits.slice(0, 4).map((b) => (
                <li key={b.title} className="border-line border bg-white p-4">
                  <p className="text-ink-900 text-sm font-bold">{b.title}</p>
                  <p className="text-mute mt-1 text-sm">{b.text}</p>
                </li>
              ))}
            </ul>
          </div>
          <div>
            <SectionHeader
              eyebrow="Eligibility you should pre-check"
              title="Qualify the lead before you log it"
              lede="Files that fit policy convert. A minute of pre-qualification saves everyone a week."
            />
            ...
            <p className="text-mute mt-4 text-sm">
              Full documentation lists are on the{" "}
              <Link
                href={`/loans/${p.slug}#documents`}
                className="text-brass-600 font-bold underline underline-offset-4"
              >
                borrower product page
              </Link>
              .
            </p>
          </div>
      ...
      <Section tone="cream" id="register">
        <div className="grid gap-10 lg:grid-cols-[1fr_0.8fr]">
          <div>
            <SectionHeader
              eyebrow="Apply"
              title={`Register as a ${p.name.toLowerCase()} DSA`}
              lede="Free, with no obligation. Pick the products you want to distribute; you can add more later."
            />
          ...
          <div className="space-y-6 lg:sticky lg:top-28 lg:self-start">
            ...
            <LinkPills
              title="Other product programmes"
              links={others.map((x) => ({ label: `${x.name} DSA`, href: `/partner/${x.slug}-dsa` }))}
            />
          </div>
        </div>
      </Section>
      ...
      <CtaBand
        title={`Start distributing ${p.name.toLowerCase()}s.`}
        lede="Free registration, one code for our whole lender panel, and a credit desk that processes every file."
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="border border-white/10 bg-white/5 p-5">
      ...
      <p className="tnum font-display mt-1 text-2xl text-white">{value}</p>
    </div>
  );
}
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="border-line border bg-white p-4">
      <dt className="text-mute text-xs font-bold tracking-wide uppercase">{label}</dt>
      <dd className="text-ink-800 mt-1 text-sm">{value}</dd>
    </div>
  );
}

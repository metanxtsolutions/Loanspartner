...
const tools = [
  {
    ...
    text: "Instalment, total interest and amortisation for any loan.",
  },
  {
    icon: ScanSearch,
    label: "Eligibility calculator",
    href: "/tools/eligibility-calculator",
    text: "How much you can borrow on your income and existing EMIs.",
  },
  {
    icon: Sparkles,
    label: "Balance transfer calculator",
    href: "/tools/balance-transfer-calculator",
    text: "Whether switching your home loan actually saves money.",
  },
];
  ...
  return (
    <>
      <JsonLd
        data={[
          ...
          itemListSchema({
            name: "Loan products",
            path: "/loans",
            items: products.map((p) => ({ name: p.name, path: `/loans/${p.slug}` })),
          }),
        ]}
      />
      <Hero />
      ...
      <Section tone="cream" id="loans">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <SectionHeader
            ...
            title={<>Twelve products. One desk that knows which lender wants your file.</>}
            lede="From a ₹50,000 personal loan to a ₹15 crore loan against property, we arrange retail and business credit across banks, housing finance companies and NBFCs."
          />
          <ButtonLink href="/loans" variant="secondary" className="shrink-0">
            All loan products <ArrowRight className="size-4" />
          </ButtonLink>
        </div>
        ...
        <div className="grid gap-12 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
          <div className="lg:sticky lg:top-28">
            <SectionHeader
              ...
              title="From enquiry to disbursal, with a person on your side at every step."
              lede="Applying everywhere at once gets you five hard enquiries and the worst rate. Applying once, to the right lender, with a complete file, gets you the best one."
            />
      ...
      <Section tone="cream">
        <SectionHeader
          align="center"
          eyebrow="Why a partner, not a portal"
          title="Portals list rates. Branches sell one product. We do the work in between."
          lede="Rate comparison is the easy part. Getting a specific profile approved by a specific lender on the best terms is a craft, and it is what our credit desk does every day."
        />
        <div className="mt-10">
          <CompareTable />
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          {tools.map((t, i) => (
            <Link
              ...
              className="group rounded-card border-line shadow-soft hover:shadow-lift border bg-white p-6 transition-all hover:-translate-y-1"
            >
              <t.icon className="text-brass-600 size-7" />
              <p className="font-display text-ink-950 group-hover:text-brass-600 mt-4 text-xl">{t.label}</p>
              <p className="text-mute mt-2 text-sm">{t.text}</p>
            </Link>
          ))}
        </div>
      </Section>
      ...
      <Section tone="ink">
        <div className="grid gap-10 lg:grid-cols-[1fr_1.2fr] lg:items-center">
          <div>
            <SectionHeader
              tone="dark"
              eyebrow="Local desks, national panel"
              title="Every city has its own property rules, employer lists and lender appetites. We know them."
              lede="From society NOCs in Mumbai to khata checks in Bengaluru and patta in Chennai, our city desks prepare files for the lender that funds that property, that profile, in that market."
            />
            <ButtonLink href="/cities" variant="light" className="mt-8">
              <MapPin className="size-4" /> Cities we serve
            </ButtonLink>
          </div>
          <ul className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
            {cities.map((c, i) => (
              <li key={c.slug} data-reveal data-reveal-delay={(i % 4) * 50}>
                <Link
                  href={`/cities/${c.slug}`}
                  className="hover:border-brass-400/60 block border border-white/10 bg-white/5 px-4 py-3 text-sm font-bold text-white/85 transition-colors hover:bg-white/10 hover:text-white"
                >
                  {c.name}
        ...
        <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <ButtonLink href="/partner/register">
                Register free <ArrowRight className="size-4" />
              </ButtonLink>
              <ButtonLink href="/partner/commission" variant="ghost">
                See commission slabs
              </ButtonLink>
            </div>
          </div>
            ...
            {partnerAudiences.map((a, i) => (
              <li key={a.slug} data-reveal data-reveal-delay={(i % 2) * 70}>
                <Link
                  ...
                  className="group rounded-card border-line hover:border-brass-400/60 hover:shadow-soft flex h-full items-start gap-3 border bg-white p-4 transition-all"
                >
                  <Users className="text-brass-600 mt-0.5 size-5 shrink-0" />
                  <span>
                    <span className="text-ink-900 group-hover:text-brass-600 block text-sm font-bold">{a.name}</span>
      ...
      <Section tone="paper">
        <SectionHeader
          ...
          title="Regulated lenders, written terms, and a desk that never asks you for money."
        />
        ...
        <Card className="mt-6 flex flex-col gap-4 p-6 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-3">
            <Landmark className="text-brass-500 mt-0.5 size-5 shrink-0" />
            <p className="text-ink-800 text-sm">
              <span className="font-bold">A note on fraud.</span> Fake agents sometimes use lender and distributor names
              to demand upfront fees. We never do. If anyone asks you to pay for approval in our name,{" "}
              <Link
                href="/guides/how-to-spot-loan-fraud"
          ...
          <Pill tone="brass" className="shrink-0">
            Never pay a fee
          </Pill>
        </Card>
      </Section>
      ...
      <Section tone="cream">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          ...
          <ButtonLink href="/guides" variant="ghost" className="shrink-0">

...
const description =
  "Compare 12 loan products from banks and NBFCs on our panel with LoansPartner: personal loans from 10.25%, home loans from 7.35%, business loans, loan against property, car and education loans. Zero fee to borrowers.";
...
export default function LoansHubPage() {
  return (
    <>
      ...
      <Section tone="ink" className="!py-14">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[720px] text-sm">
            <caption className="font-display pb-4 text-left text-2xl text-white">At a glance</caption>
            <thead className="text-left text-xs tracking-wide text-white/50 uppercase">
              <tr>
            ...
            <tbody className="tnum">
              {products.map((p) => (
                <tr key={p.slug} className="border-t border-white/10">
                  <th scope="row" className="py-3 pr-4 text-left font-bold text-white">
                    <Link href={`/loans/${p.slug}`} className="hover:text-brass-400">
                      {p.name}
                    </Link>
                  </th>
      ...
      <Section tone="paper">
        <SectionHeader
          eyebrow="Loans by city"
          title="City desks for the markets we know best."
          lede="Property rules, employer categories and lender appetite differ by city. Each city page explains what changes locally."
        />
        <ul className="mt-8 flex flex-wrap gap-2">
          {cities.map((c) => (
            <li key={c.slug}>
              <Link
                href={`/cities/${c.slug}`}
                className="border-line text-ink-800 hover:border-brass-500 hover:text-brass-600 inline-flex items-center gap-1 border bg-white px-3.5 py-1.5 text-[13px] font-semibold"
              >
                Loans in {c.name}
              </Link>
            </li>
          ...
          <li>
            <Link
              href="/cities"
              className="bg-ink-900 inline-flex items-center gap-1 px-3.5 py-1.5 text-[13px] font-semibold text-white"
            >
              All cities <ArrowRight className="size-3.5" />
            </Link>
          </li>
        </ul>

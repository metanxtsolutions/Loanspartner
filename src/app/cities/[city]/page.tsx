  ...
  return (
    <>
      <JsonLd data={[
        ...
        itemListSchema({ name: `Loans in ${c.name}`, path, items: coreProducts.map((p) => ({ name: `${p.name} in ${c.name}`, path: `/loans/${p.slug}/${c.slug}` })) }),
        faqPageSchema(c.faqs),
      ]} />
      <PageHero
        tone="ink"
        crumbs={[{ name: "Cities", path: "/cities" }, { name: c.name, path }]}
        ...
        aside={
          <div className="rounded-panel bg-white p-6 text-ink-900 shadow-lift">
            ...
            <p className="mt-1 text-sm text-mute">Tell us the need; we call back with lenders and documents to keep ready.</p>
            <div className="mt-4"><CallbackForm city={c.name} source={`city-callback:${c.slug}`} /></div>
          </div>
        }
      />

      <Section tone="cream">
        <SectionHeader eyebrow="Products" title={`What we arrange in ${c.name}`} lede="Each product page explains what changes locally: documentation, lender appetite and typical ticket sizes." />
        <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {coreProducts.map((p, i) => (<ProductCard key={p.slug} product={p} href={`/loans/${p.slug}/${c.slug}`} cityName={c.name} delay={i * 60} />))}
        </div>
        <LinkPills className="mt-8" title={`Also available in ${c.name}`} links={otherProducts.map((p) => ({ label: p.name, href: `/loans/${p.slug}` }))} />
      </Section>
            ...
            {nearby.length > 0 && (
              <Card className="p-5">
                ...
                <ul className="mt-3 space-y-2 text-sm">
                  {nearby.map((n) => (<li key={n.slug}><Link href={`/cities/${n.slug}`} className="inline-flex items-center gap-1 font-semibold text-ink-800 hover:text-verdant-700">Loans in {n.name} <ArrowRight className="size-3.5" /></Link></li>))}
                </ul>
              </Card>
            )}
          </aside>
      ...
      <Section tone="paper">
        <TrustNotes />
        <LinkPills className="mt-12" title="Other cities" links={cities.filter((x) => x.slug !== c.slug).map((x) => ({ label: x.name, href: `/cities/${x.slug}` }))} />
      </Section>

      <CtaBand title={`Borrowing in ${c.name}? Start with a two-minute check.`} lede={`Our ${c.name} desk pre-screens your profile across the panel and calls with a shortlist. No fee, no bureau enquiry.`} primary={{ label: "Check eligibility", href: `/apply?city=${encodeURIComponent(c.name)}` }} />
    </>
  );
}

    ...
    <>
      ...
      >
        <div className="mt-6 flex flex-wrap gap-2">
          <Pill tone="verdant">Rates from {p.rate.from.toFixed(2)}% p.a.</Pill>
        ...
        <div className="grid gap-12 lg:grid-cols-[minmax(0,1fr)_18rem]">
          <div className="space-y-16">
            <div className="prose-lp max-w-3xl" data-reveal>
              <h2>The {c.name} market for {p.name.toLowerCase()}s</h2>
              {c.overview.map((para, i) => (<p key={i}>{para}</p>))}
              {(p.slug === "home-loan" || p.slug === "loan-against-property") && (<><h3>Property in {c.name}</h3><p>{c.propertyMarket}</p></>)}
              {(p.slug === "business-loan" || p.slug === "personal-loan") && (<><h3>Who borrows in {c.name}</h3><p>{c.economy}</p></>)}
            ...
            <div>
              ...
              <Card className="mt-6 p-6"><CheckList items={c.localNotes} /></Card>
            </div>
            ...
            <div>
              <SectionHeader eyebrow="Indicative EMIs" title="Instalments at a glance" lede={<>At the lowest rate in the range. Run your own numbers in the <Link href="/tools/emi-calculator" className="font-bold text-verdant-700 underline underline-offset-4">EMI calculator</Link>.</>} />
            ...
            <div>
              ...
              <div className={`mt-8 grid gap-6 ${p.documents.salaried.length ? "md:grid-cols-2" : ""}`}>
                {p.documents.salaried.length > 0 && (<Card className="p-6"><Pill tone="verdant">Salaried</Pill><CheckList items={p.documents.salaried} className="mt-4" /></Card>)}
                <Card className="p-6"><Pill tone="brass">{p.documents.salaried.length ? "Self-employed" : "Business"}</Pill><CheckList items={p.documents.selfEmployed} className="mt-4" /></Card>
              </div>
            </div>

            {lenders.length > 0 && (
              <div>
                <SectionHeader eyebrow="Lenders" title={`${p.name} lenders active in ${c.name}`} lede="A selection from our panel. Lender choice depends on your profile and, for property loans, the specific property." />
          ...
          <aside className="space-y-6 lg:sticky lg:top-28 lg:self-start">
            <Card className="p-5">
              ...
              <ul className="mt-3 space-y-2 text-sm text-ink-800">
                <li className="flex gap-2"><MapPin className="mt-0.5 size-4 shrink-0 text-verdant-600" /> Serving {c.name} and the wider {c.state} region</li>
                <li className="flex gap-2"><Landmark className="mt-0.5 size-4 shrink-0 text-verdant-600" /> {lenders.length} panel lenders for this product</li>
                <li className="flex gap-2"><Building2 className="mt-0.5 size-4 shrink-0 text-verdant-600" /> Local documentation and verification support</li>
              </ul>
              <Link href={`/cities/${c.slug}`} className="mt-4 inline-flex items-center gap-1 text-sm font-bold text-verdant-700">All loans in {c.name} <ArrowRight className="size-3.5" /></Link>
            </Card>
            <Card className="p-5">
              <p className="eyebrow text-mute">Other loans in {c.name}</p>
              <ul className="mt-3 space-y-2 text-sm">
                {otherProducts.map((x) => (<li key={x.slug}><Link href={`/loans/${x.slug}/${c.slug}`} className="font-semibold text-ink-800 hover:text-verdant-700">{x.name} in {c.name}</Link></li>))}
              </ul>
            </Card>
            {nearby.length > 0 && (
              <Card className="p-5">
                ...
                <ul className="mt-3 space-y-2 text-sm">
                  {nearby.map((x) => (<li key={x.slug}><Link href={`/loans/${p.slug}/${x.slug}`} className="font-semibold text-ink-800 hover:text-verdant-700">{p.name} in {x.name}</Link></li>))}
                </ul>
              </Card>
            )}
          </aside>
      ...
      <Section tone="paper">
        <TrustNotes />
        <LinkPills className="mt-12" title={`${p.name}s in other cities`} links={otherCities.map((x) => ({ label: x.name, href: `/loans/${p.slug}/${x.slug}` }))} />
      </Section>

      <CtaBand title={`Your ${p.name.toLowerCase()} in ${c.name}, handled.`} lede={`Share your requirement and our ${c.name} desk will call with a lender shortlist and the documents to keep ready. No fee, no bureau enquiry.`} primary={{ label: "Check eligibility", href: `/apply?product=${p.slug}&city=${encodeURIComponent(c.name)}` }} />
    </>
  );
}

...
export function ProofStrip({ tone = "light", className }: { tone?: "light" | "dark"; className?: string }) {
  return (
    <dl className={cn("grid grid-cols-2 gap-6 lg:grid-cols-4", className)}>
      {proof.map((p, i) => (
        <div key={p.label} data-reveal data-reveal-delay={i * 80} className="border-brass-400 border-l-2 pl-5">
          <dd className={cn("display tnum text-4xl lg:text-5xl", tone === "dark" ? "text-white" : "text-ink-950")}>
            {p.value}
          </dd>

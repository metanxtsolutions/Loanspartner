...
/**
 * Brand marks are inline because lucide dropped its brand icon set in v1.
 * Paths are the standard 24x24 brand glyphs, used only to link to the
 * company's own official profiles.
 */
...
export function SocialLinks({ className, tone = "dark" }: { className?: string; tone?: "dark" | "light" }) {
  const profiles = Object.entries(siteConfig.social).filter(([key, href]) => href && marks[key]);
        ...
        return (
          <li key={key}>
            <a
              ...
              aria-label={`${siteConfig.name} on ${mark.label}`}
              className={cn(

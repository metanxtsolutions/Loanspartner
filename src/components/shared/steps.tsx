...
export function Steps({
  ...
  steps: readonly (string | { name: string; text: string })[];
  tone?: "light" | "dark";
  columns?: boolean;
}) {
  return (
      ...
      {steps.map((s, i) => {
        ...
        return (
          <li key={i} data-reveal data-reveal-delay={i * 70} className="flex gap-4">
            <span
              className={cn(
                "display flex size-11 shrink-0 items-center justify-center border text-lg",
                tone === "dark" ? "border-white/20 text-white" : "border-line text-ink-800 bg-white",
              )}
            >
            ...
            <div className="pt-1.5">
              ...
              <p
                className={cn(
                  ...
                  tone === "dark" ? "text-white/70" : "text-mute",
                  name && "mt-1",
                )}
              >
                {text}

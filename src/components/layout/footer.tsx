      ...
      <div className="container-x">
        <div className="grid gap-10 border-b border-white/10 py-14 lg:grid-cols-12">
          <div className="lg:col-span-4">
            ...
            <ul className="mt-6 space-y-2.5 text-sm">
              <li className="flex items-start gap-2.5 text-white/80">
                <Phone className="text-brass-400 mt-0.5 size-4 shrink-0" />
                <a href={`tel:${siteConfig.contact.phone}`} className="hover:text-white">
                  {siteConfig.contact.phoneDisplay}
                </a>
              </li>
              <li className="flex items-start gap-2.5 text-white/80">
                <Mail className="text-brass-400 mt-0.5 size-4 shrink-0" />
                <a href={`mailto:${siteConfig.contact.email}`} className="hover:text-white">
                  {siteConfig.contact.email}
                </a>
              </li>
              <li className="flex items-start gap-2.5 text-white/80">
                <MapPin className="text-brass-400 mt-0.5 size-4 shrink-0" />
          ...
          <FooterCol title="Loans" className="lg:col-span-2">
            {products.map((p) => (
              <FooterLink key={p.slug} href={`/loans/${p.slug}`}>
                {p.name}
              </FooterLink>
            ))}
          </FooterCol>
        ...
        <div className="flex flex-col gap-4 border-t border-white/10 py-6 text-xs text-white/60 md:flex-row md:items-center md:justify-between">
          <p className="flex items-center gap-2">
            <ShieldAlert className="text-brass-400 size-4" /> We never charge borrowers. Never pay anyone for loan
            approval in our name.
          </p>
          <p>
...
function FooterCol({ title, children, className }: { title: string; children: React.ReactNode; className?: string }) {
  return (
    <div className={className}>
      <p className="eyebrow text-brass-400 mb-4">{title}</p>

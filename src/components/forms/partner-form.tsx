...
export function PartnerForm({
  ...
  if (state.ok) {
    return (
      <div className="space-y-4">
        ...
        <ol className="text-ink-800 list-decimal space-y-1 pl-5 text-sm">
  ...
  return (
    <form
      onSubmit={onSubmit}
      method="post"
      aria-label="Channel partner application"
    ...
    >
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Field label="Mobile" name="phone" error={state.errors?.phone}>
          {(a) => (
            <input {...a} name="phone" type="tel" inputMode="tel" autoComplete="tel" className="field" required />
          )}
        </Field>
        ...
        <Field label="Your profession" name="profession" error={state.errors?.profession}>
          {(a) => (
            <select {...a} name="profession" className="field" defaultValue={defaultProfession ?? ""}>
              ...
              {audiences.map((x) => (
                <option key={x.slug} value={x.name}>
                  {x.name}
                </option>
              ))}
        ...
        <Field
          label="Experience with loan distribution"
          name="experience"
      ...
      <fieldset>
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
          {products.map((p) => (
            <label
              key={p.slug}
              className="border-line has-checked:border-brass-500 has-checked:bg-brass-50 has-[:focus-visible]:outline-brass-500 flex cursor-pointer items-center gap-2 border bg-white px-3 py-2 text-sm font-semibold has-[:focus-visible]:outline-2 has-[:focus-visible]:outline-offset-2"
            >
              <input
                type="checkbox"
                name="products[]"
                value={p.slug}
                className="accent-brass-500 size-4"
      ...
      <Field label="Tell us about your network (optional)" name="network" error={state.errors?.network}>
        {(a) => (
          <textarea
            ...
            rows={3}
            className="field"
            placeholder="Who are your clients or contacts, and roughly how many loan needs do you see in a month?"
          />
        )}
      </Field>
      <label className="text-mute flex items-start gap-3 text-xs">
        ...
        <span>
          I confirm the details are accurate and agree to be contacted about the partner programme. I understand
          partners never collect money from borrowers and must follow the LoansPartner code of conduct.
        </span>
      </label>
      ...
      <SubmitButton pending={pending}>
        Submit application <ArrowRight className="size-4" aria-hidden />
      </SubmitButton>
    </form>
  );
}

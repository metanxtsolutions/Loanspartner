    ...
    <form onSubmit={onSubmit} method="post" aria-label="Request a callback" className="relative space-y-3" noValidate>
      ...
      {!compact && (
        <Field label="Anything we should know? (optional)" name="cb-note" error={state.errors?.note}>
          {(a) => (
            <textarea {...a} name="note" rows={2} className="field" placeholder="Amount, timeline, property type..." />
          )}
        </Field>
      )}
      <FormError message={state.message} />
      <SubmitButton pending={pending}>
        <PhoneCall className="size-4" aria-hidden /> Request a callback
      </SubmitButton>
      <p className="text-mute text-center text-[11px]">We call between 9:30 am and 6 pm, Monday to Saturday.</p>
    </form>
  );
}

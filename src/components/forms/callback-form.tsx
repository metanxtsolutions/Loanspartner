  ...
  return (
    <form onSubmit={onSubmit} className="relative space-y-3" noValidate>
      ...
      <Field label="Name" name="cb-name" error={state.errors?.name}>
        <input id="cb-name" name="name" type="text" autoComplete="name" className="field" placeholder="Your name" required />
      </Field>
      <Field label="Mobile" name="cb-phone" error={state.errors?.phone}>
        <input id="cb-phone" name="phone" type="tel" inputMode="tel" autoComplete="tel" className="field" placeholder="10-digit mobile" required />
      </Field>
      {!compact && (
        <Field label="Anything we should know? (optional)" name="cb-note" error={state.errors?.note}>
          <textarea id="cb-note" name="note" rows={2} className="field" placeholder="Amount, timeline, property type..." />
        </Field>
      )}
      <FormError message={state.message} />
      <SubmitButton pending={pending}><PhoneCall className="size-4" /> Request a callback</SubmitButton>

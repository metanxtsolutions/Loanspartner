...
/**
 * Postgres row per lead, worked from /console/leads. The mapping lives in
 * src/lib/leads/website-lead.ts so the backfill script replays history
 * through the exact same code. A failure here throws so it is counted and
 * logged like any other sink; the email still goes out.
 */
async function persistDatabase(event: LeadEvent): Promise<boolean> {
  if (!process.env.DATABASE_URL) return false;
  await upsertWebsiteLeadFromEvent(prisma, event);
  return true;
}
